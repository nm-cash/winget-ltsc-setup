...
`winget` busca estos paquetes de repositorios oficiales revisados por Microsoft. Así que de entre los varios gestores de paquetes que existen, es el más "fiable" por así decirlo. Aun así, caben mencionar los otros dos contendientes en cuanto a gestores de paquetes para Windows, que son [`chocolatey`](https://github.com/chocolatey/choco) y [`scoop`](https://github.com/ScoopInstaller/Scoop).
Cualquiera de los tres ofrece distintos argumentos por línea de comandos mediante los cuales buscar de entre una larga serie de programas instalables, como navegadores, editores de texto, gestores de archivos, etc... 
- Buscar estos programas por palabras clave, por ejemplo tipear `winget search "browser"`. Y obtener una larga lista de navegadores web, o cualquier programa que en su descripción contenga el string:"browser", compatibles con mi sistema de entre los cuales elegir.
- Puedo buscar información sobre la proveniencia de estos programas, por ejemplo tipeando `winget info Programa.X`, para obtener datos como el repositorio o fuente de origen para su descarga, autor/es del programa, número de versión, etc...
- En general me ofrece un control granular y muy flexible sobre los programas que tengo y los que me interesa incorporar a mí sistema. Por eso es interesante contar con esta clase de herramientas en él.

`winget` no viene por defecto en LTSC, pero es una herramienta indispensable para un power-user que quiera poblar su despliegue windows con herramientas seleccionadas a medida. Para lograr esto, hay que habilitarlo.
...
wsreset -i
```
**Esto va a forzar la re-instalación de la tienda de Microsoft.** Para nuestro caso con LTSC, como la tienda no estaba instalada, lo que va a ser forzado es su primer instalación. Una vez ejecutado hay que esperar, hasta que aparezca una notificación pop-up que indique que "La tienda ha sido instalada.". Luego de eso [se recomienda instalar el instalador de aplicaciones](https://apps.microsoft.com/detail/9nblggh4nns1?hl=en-US&gl=AR), que incluye `winget`, y permite la instalación de paquetes en formato `*.appx`, como los que `winget` usa.

Cabe aclarar que `winget` y la tienda _no son lo mismo_. `winget` es capaz de sourcear programas de la tienda, sí. Pero también de los repositorios comunitarios de `winget`. Por esto que su cobertura es más amplia, y es una herramienta más poderosa que la tienda. Sin embargo, no son excluyentes entre sí. Es conveniente contar con ambas opciones. En la mayoría de los casos me gusta priorizar `winget` para buscar e instalar los programas que necesito, y en caso de encontrar nada, entonces si caigo sobre la Microsoft Store como recurso secundario.

En la tienda tendremos la opción de instalar [`UniGet-UI`](https://github.com/marticliment/UniGetUI). `UniGet-UI` incluye `winget`, y le da una interfaz gráfica con la cual utilizarlo. Esto es muy útil para exprimir un poco más de las capacidades adicionales del gestor de paquetes. De hecho fue el método "de entrada" que utilicé para instalar `winget` en mí sistema, al no poder encontrar el instalador de aplicaciones disponible en la ms-store.

Eso es todo. Diviértete con tu minimal Windows introduciéndole cosas a medida.
